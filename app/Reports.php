...
//            ->where('tools_request_goods.order_recieved',0)
//            ->orderBy('tools_request_goods.id','desc')
    ...
    public function getToolInventoryList($search){
        ...
        if(trim($search['search']) != ''){
            ...
            $q->select(
                'tools_category_items.*',
                'tools_category_items.id as item_id',
    ...
    public function getReturnedItemsReport($project_id){

        $q = DB::table('returned_items');
            $q->join('projects','returned_items.project_id','projects.id');
            $q->join('category_items','returned_items.item_id','category_items.id');
            $q->join('categories','category_items.category_id','categories.id');
            $q->select(
            ...
            $q->where('returned_items.is_returned',1);
            if($project_id > 0){
...
//        print_r($data);die();
    ...
    public function getReturnedToolsReports($project_id){

        $q = DB::table('returned_tools');
            $q->join('projects','returned_tools.project_id','projects.id');
            $q->join('tools_category_items','returned_tools.item_id','tools_category_items.id');
            $q->join('tools_categories','tools_category_items.category_id','tools_categories.id');
            $q->select(
            ...
            $q->where('returned_tools.is_returned',1);
            if($project_id > 0){
                $q->where('returned_tools.project_id',$project_id);
            }
            $data = $q->get();
