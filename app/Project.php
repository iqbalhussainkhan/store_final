...
//         print_r(Session('project_id'));die();
    ...
    public function allProjectsToolsInventory($project_id = 0){

        $q = DB::table('project_tools');
            $q->join('tools_category_items','project_tools.item_id','=','tools_category_items.id');
            $q->join('tools_categories','project_tools.category_id','=','tools_categories.id');
            $q->join('projects','project_tools.project_id','=','projects.id');
             if($project_id > 0){
                 $q->where('project_tools.project_id',$project_id);
             }
            $q->where('project_tools.under_store_approval','!=',1);
            $q->where('project_tools.is_recevied',0);
            $q->where('project_tools.store_return_approve',0);
            $q->select(
                'project_tools.id as project_tool_id',
                'project_tools.*',
                'tools_categories.*',
